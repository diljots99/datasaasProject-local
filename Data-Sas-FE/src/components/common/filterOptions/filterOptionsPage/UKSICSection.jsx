...
export default function UKSICSection() {
  ...
  const getOptions = () => {
    let filtervalue = filterTypeDetail.filter(
      (value) =>
        value.name === "UK SIC Section" && value.category === "Activities"
    );
    if (filtervalue) {
  ...
  const applyFilter = () => {
    if (selectedOption !== null && selectedOption.length > 0) {
      ...
      dispatch(SetselectedFilterValues("UK SIC Section", filterval));
