...
export default function CompanyCategory() {
  ...
  const getOptions = () => {
    ...
    let filtervalue = filterTypeDetail.filter(value => value.name ===  "Company Category" &&  value.category === "Status")
    if (filtervalue) {
  ...
  const applyFilter = () => {
    ...
    if( selectedOption !==null && selectedOption.length > 0){
      ...
      dispatch(SetselectedFilterValues("Company Category", filterval))
