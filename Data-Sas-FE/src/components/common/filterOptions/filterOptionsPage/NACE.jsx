...
export default function NACE() {
  ...
  const getOptions = () => {
    ...
    let filtervalue = filterTypeDetail.filter(value => value.name ===   "NACE" &&  value.category === "Activities")
    if (filtervalue) {
  ...
  const applyFilter = () => {
    if( selectedOption !==null && selectedOption.length > 0){
      ...
      dispatch(SetselectedFilterValues("NACE", filterval))
