...
export default function ReferenceNumber() {
  ...
  const getOptions = () => {
    ...
    let filtervalue = filterTypeDetail.filter(
      (value) =>
        value.name === "Reference Number" && value.category === "Company"
    );
    if (filtervalue) {
  ...
  useEffect(() => {
    if(selectedFilterValues["Reference Number"]){
      setChecked(selectedFilterValues["Reference Number"])
    }else{
      setChecked([])
    }
  ...
  const applyFilter = () => {
    if (checked.length > 0) {
      dispatch(SetselectedFilterValues("Reference Number", checked));
    }
  };
  return (
