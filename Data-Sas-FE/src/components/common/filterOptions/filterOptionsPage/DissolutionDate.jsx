...
export default function DissolutionDate() {
  ...
  const getOptions = () => {
    let filtervalue = filterTypeDetail.filter(
      (value) =>
        value.name === "Dissolution Date" && value.category === "Status"
    );
    console.log("Dissolution Date val ", filtervalue);
  ...
  const applyFilter = () => {
    ...
    if (startDate && endDate) {
      dispatch(
        SetselectedFilterValues("Dissolution Date", [ moment(startDate).format(),  moment(endDate).format()])
      );
      setStartDate(null);
      setEndDate(null);
      setError(false);
