...
export default function Address() {
  ...
  const getOptions = () => {
    ...
    let filtervalue = filterTypeDetail.filter(
      (value) =>
        value.name === "Address" && value.category === "Company"
    );
    if (filtervalue) {
  ...
  useEffect(() => {
      if(selectedFilterValues.Address){
        setChecked(selectedFilterValues.Address)
      }else{
        setChecked([])
      }
  ...
  const applyFilter = () => {
    if (checked.length > 0) {
      dispatch(SetselectedFilterValues("Address", checked));
    }
  };
