...
export default function Telephone() {
  ...
  const getOptions = () => {
    ...
    let filtervalue = filterTypeDetail.filter(
      (value) =>
        value.name === "Telephone" && value.category === "Company"
    );
    if (filtervalue) {
  ...
  useEffect(() => {
    if(selectedFilterValues["Telephone"]){
      setChecked(selectedFilterValues["Telephone"])
    }else{
      setChecked([])
    }
  ...
  const applyFilter = () => {
    if (checked.length > 0) {
      dispatch(SetselectedFilterValues("Telephone", checked));
    }
  };
  return (
