...
const DirectorWatch = () => {
  ...
  useEffect(() => {
    if (searchKey) {
      ...
      let results = directorList.filter((company) => company.name.match(re));
    ...
    ({ pageSize, pageIndex }) => {
      ...
      setTimeout(() => {
        if (fetchId === directorfetchIdRef.current) {
          ...
          settableData(directorList.slice(startRow, endRow));
          setPageCount(Math.ceil(directorList.length / pageSize));
          setLoading(false);
        }
      }, 3000);
    },
